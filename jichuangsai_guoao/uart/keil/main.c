...
{ 
	...
	while(1)
	{
		...
		if(waterlight_mode == 1) UARTString(" Cortex-M0 Start up!WaterLight mode setting to left mode!\n");
		else if (waterlight_mode == 2) UARTString("Cortex-M0 Start up!WaterLight mode setting to right mode!\n");
		else if (waterlight_mode == 3) UARTString("Cortex-M0 Start up!WaterLight mode setting to flash mode!\n");
